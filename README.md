...
1. 双击 **`启动Tocify.bat`**（首次运行会自动：
   ...
   - 然后自动启动服务）
2. 服务启动成功后，**自动打开浏览器**访问 `http://localhost:5173`
3. 若没自动打开，可手动在浏览器里输入 `http://localhost:5173`

> **关于窗口**：双击脚本后会弹出**两个窗口**——
> - 第一个（启动器）会**一闪而过**，这是正常的，它只是负责启动服务；
> - 第二个标题为 **「Tocify 服务窗口（关闭此窗口即停止服务）」** 的黑色窗口会**一直保持打开**，它就是服务本体。
>
> **停止服务**：关闭上面那个「Tocify 服务窗口」即可。如果关掉后浏览器还能打开页面，说明还有旧的服务在运行，把旧窗口也关掉即可。
...
- 下拉选择 **"Custom / OpenAI Compatible"**（自定义 OpenAI 兼容接口）
- 支持：Gemini、Qwen、Doubao、Zhipu、自定义聚合平台（如 OpenCodeGo、Command Code 等）
...
- 如 `gpt-4o`、`qwen-vl-plus` 等（取决于你的 Provider 支持的模型）
...
3. 如果目录分布在不连续的页面，可以点击 **"Add another range"** 添加多个范围（如第 1-2 页 + 第 5-6 页）。
...
2. 等待 AI 识别（通常几秒到几分钟，取决于页数和目录长度）。
   - 若目录页数较多，会自动分批识别，进度条会显示进度。
3. 识别完成后，左侧编辑器会出现目录条目标题和页码。
...
1. 点击左侧目录列表中**第一个目录项**（它会触发右侧预览跳到对应页）。
2. 页面会弹出 **"通过首个目录项校准页码偏移"** 窗口。
3. 在右侧预览中核对，选择该目录项**实际对应的物理页码**。
4. 点击 **"是的！就是这一页！"**。

> 也可以手动在 **"目录设置" → "页码偏移量"** 中调整（物理页码 - 标注页码 = 偏移量）。
...
3. 用阅读器（如 Adobe Reader、掌阅、WPS）打开，点击目录书签即可跳转。
